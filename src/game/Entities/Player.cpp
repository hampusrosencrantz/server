...
int32 Player::getMaxTimer(MirrorTimerType timer) const
{
    switch (timer)
    {
        ...
        case BREATH_TIMER:
        {
            if (!isAlive() || HasAuraType(SPELL_AURA_WATER_BREATHING) || !GodModeCheat)
                return DISABLED_MIRROR_TIMER;
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_AFK | PLAYER_FLAGS_DND | PLAYER_FLAGS_GM | PLAYER_FLAGS_GHOST | PLAYER_FLAGS_PVP_DESIRED | PLAYER_FLAGS_PVP_TIMER);
...
void Player::_LoadSpellCooldowns(QueryResult* result)
{
    ...
    // QueryResult *result = CharacterDatabase.PQuery("SELECT SpellId, SpellExpireTime, Category, CategoryExpireTime, ItemId FROM character_spell_cooldown WHERE LowGuid = '%u'",GetGUIDLow());
...
void Player::_SaveSpellCooldowns()
{
    ...
    SqlStatement stmt = CharacterDatabase.CreateStatement(deleteSpellCooldown, "DELETE FROM character_spell_cooldown WHERE LowGuid = ?");
    ...
    for (auto& cdItr : m_cooldownMap)
    {
        ...
        if (!cdData->IsPermanent())
        {
            ...
            stmt = CharacterDatabase.CreateStatement(insertSpellCooldown, "INSERT INTO character_spell_cooldown (LowGuid, SpellId, SpellExpireTime, Category, CategoryExpireTime, ItemId) VALUES( ?, ?, ?, ?, ?, ?)");
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder* holder)
{
    ...
    //"health, power1, power2, power3, power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, banned, banReason  FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
	...
	m_banned = fields[65].GetUInt32();
	m_banreason = fields[66].GetString();
...
void Player::SaveToDB()
{
    ...
    SqlStatement uberInsert = CharacterDatabase.CreateStatement(insChar, "INSERT INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
                              ...
                              "power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, banned, banReason) "
                              "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
                              "?, ?, ?, ?, ?, ?, "
                              "?, ?, ?, "
                              "?, ?, ?, ?, ?, ?, ?, "
	...
	uberInsert.addUInt32(m_banned);
	uberInsert.addString(m_banreason);
...
void Player::UpdatePotionCooldown(Spell* spell)
{
    ...
    else
    {
        ...
        for (int idx = 0; idx < 5; ++idx)
            if (itemProto->Spells[idx].SpellId && itemProto->Spells[idx].SpellTrigger == ITEM_SPELLTRIGGER_ON_USE)
                spellEntry = sSpellTemplate.LookupEntry<SpellEntry>(itemProto->Spells[idx].SpellId);
    }
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
    ...
    if (z_diff >= 14.57f && !isDead() && !GodModeCheat && !HasMovementFlag(MOVEFLAG_ONTRANSPORT) &&
...
bool Player::IsPetNeedBeTemporaryUnsummoned() const
{
    ...
    if (HasAuraType(SPELL_AURA_FLY) || (IsMounted() && HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED))) // if not IsMounted and has aura, means we are removing flying mount
        return true;
...
void Player::SendEquipmentSetList()
{
    ...
    {
        EquipmentSet const& equipmentSet = itr->second;
