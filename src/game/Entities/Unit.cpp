    ...
    m_spellUpdateHappening(false)
{
    ...
    m_modDodgeChanceDiminishing = 0.0f;
    m_modParryChance = 0.0f;
    m_modParryChanceDiminishing = 0.0f;
    ...
    for (AuraList::const_iterator itr = vShareDamageAuras.begin(); itr != vShareDamageAuras.end(); ++itr)
    {
        ...
        if (Unit* shareTarget = (*itr)->GetCaster())
        {
            if (shareTarget != pVictim && ((*itr)->GetMiscValue() & damageSchoolMask))
            {
                ...
                DealDamageMods(shareTarget, shareDamage, nullptr, damagetype);
...
void Unit::CalculateMeleeDamage(Unit* pVictim, CalcDamageInfo* calcDamageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/)
{
    ...
    calcDamageInfo->damageSchoolMask = GetMeleeDamageSchoolMask();
    calcDamageInfo->attackType       = attackType;
    calcDamageInfo->damage           = 0;
    calcDamageInfo->cleanDamage      = 0;
    calcDamageInfo->absorb           = 0;
    calcDamageInfo->resist           = 0;
    ...
    if (calcDamageInfo->target->IsImmuneToDamage(calcDamageInfo->damageSchoolMask))
    {
        calcDamageInfo->HitInfo       |= HITINFO_NORMALSWING;
    ...
    uint32 damage = CalculateDamage(calcDamageInfo->attackType, false);
    // Add melee damage bonus
    damage = MeleeDamageBonusDone(calcDamageInfo->target, damage, calcDamageInfo->attackType);
    damage = calcDamageInfo->target->MeleeDamageBonusTaken(this, damage, calcDamageInfo->attackType);

    // Calculate armor reduction if physical
    SpellSchoolMask schoolMask = GetMeleeDamageSchoolMask();
    if (schoolMask == SPELL_SCHOOL_MASK_NORMAL)
    {
        uint32 armor_affected_damage = CalcNotIgnoreDamageReduction(damage, calcDamageInfo->damageSchoolMask);
        calcDamageInfo->damage = damage - armor_affected_damage + static_cast<int>(CalcArmorReducedDamage(calcDamageInfo->target, damage) + 0.5); // Round the results
        calcDamageInfo->cleanDamage += damage - calcDamageInfo->damage;
    }

    calcDamageInfo->hitOutCome = RollMeleeOutcomeAgainst(calcDamageInfo->target, calcDamageInfo->attackType, calcDamageInfo->damageSchoolMask);
    ...
    switch (calcDamageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_BLOCK:
        {
            ...
            if (calcDamageInfo->blocked_amount >= calcDamageInfo->damage)
            {
                calcDamageInfo->TargetState = VICTIMSTATE_BLOCKS;
                calcDamageInfo->blocked_amount = calcDamageInfo->damage;
                calcDamageInfo->procEx |= PROC_EX_FULL_BLOCK;
            }
            else
                calcDamageInfo->procEx |= PROC_EX_NORMAL_HIT;   // Partial blocks can still cause attacker procs

            calcDamageInfo->damage      -= calcDamageInfo->blocked_amount;
        ...
        case MELEE_HIT_CRUSHING:
        {
            ...
            calcDamageInfo->procEx |= PROC_EX_NORMAL_HIT;
            // 150% normal damage
            calcDamageInfo->damage += (calcDamageInfo->damage / 2);
            break;
        }
        default:
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        uint32 reduction_affected_damage = CalcNotIgnoreDamageReduction(calcDamageInfo->damage, calcDamageInfo->damageSchoolMask);
        ...
        calcDamageInfo->damage      -= resilienceReduction;
        calcDamageInfo->cleanDamage += resilienceReduction;
    }

    // Calculate absorb resist
    if (int32(calcDamageInfo->damage) > 0)
    {
        calcDamageInfo->procVictim |= PROC_FLAG_TAKEN_ANY_DAMAGE;

        // Calculate absorb & resists
        uint32 absorb_affected_damage = CalcNotIgnoreAbsorbDamage(calcDamageInfo->damage, calcDamageInfo->damageSchoolMask);
        calcDamageInfo->target->CalculateDamageAbsorbAndResist(this, calcDamageInfo->damageSchoolMask, DIRECT_DAMAGE, absorb_affected_damage, &calcDamageInfo->absorb, &calcDamageInfo->resist, true);
        calcDamageInfo->damage -= calcDamageInfo->absorb + calcDamageInfo->resist;
        if (calcDamageInfo->absorb)
        {
            calcDamageInfo->HitInfo |= HITINFO_ABSORB;
            calcDamageInfo->procEx |= PROC_EX_ABSORB;
        }
        if (calcDamageInfo->resist)
        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                if (spellProto->Id == 50461)
                {
                    ...
                    pCaster->DealDamageMods(caster, ab_damage, nullptr, damagetype);
...
bool Unit::CanDodgeAbility(const Unit* attacker, const SpellEntry* ability) const
{
    ...
    if (!CanDodgeInCombat(attacker))
        return false;
    // Check if attacker cuts through dodge with this ability
    const AuraList& ignore = attacker->GetAurasByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (AuraList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
        if ((*i)->isAffectedOnSpell(ability) && (*i)->GetModifier()->m_miscvalue == MELEE_HIT_DODGE)
            return false;
    }
    return true;
}
...
bool Unit::CanParryAbility(const Unit* attacker, const SpellEntry* ability) const
{
    ...
    if (!CanParryInCombat(attacker))
        return false;
    // Check if attacker cuts through parry with this ability
    const AuraList& ignore = attacker->GetAurasByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (AuraList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
        if ((*i)->isAffectedOnSpell(ability) && (*i)->GetModifier()->m_miscvalue == MELEE_HIT_PARRY)
            return false;
    }
    return true;
}
...
bool Unit::CanBlockAbility(const Unit* attacker, const SpellEntry* ability, bool miss) const
{
    ...
    if (!CanBlockInCombat(attacker))
        return false;
    // Check if attacker cuts through block with this ability
    const AuraList& ignore = attacker->GetAurasByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (AuraList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
        if ((*i)->isAffectedOnSpell(ability) && (*i)->GetModifier()->m_miscvalue == MELEE_HIT_BLOCK)
            return false;
    }
    return true;
}
...
bool Unit::CanDeflectAbility(const Unit* attacker, const SpellEntry* ability) const
{
    ...
    if (!CanParryInCombat(attacker))
        return false;
    // Check if attacker cuts through parry/deflect with this ability
    const AuraList& ignore = attacker->GetAurasByType(SPELL_AURA_IGNORE_COMBAT_RESULT);
    for (AuraList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
        if ((*i)->isAffectedOnSpell(ability) && (*i)->GetModifier()->m_miscvalue == MELEE_HIT_PARRY)
...
static const float AVOIDANCE_DIMINISHING_COEFFICIENT[MAX_CLASSES] =
{
    0.0000f,  // [0]  <Unused>
    0.9560f,  // [1]  Warrior
    0.9560f,  // [2]  Paladin
...
static const float AVOIDANCE_DIMINISHING_DODGE_CAP[MAX_CLASSES] =
{
    0.000000f, // [0]  <Unused>
    88.129021f, // [1]  Warrior
    88.129021f, // [2]  Paladin
    145.560408f, // [3]  Hunter
    145.560408f, // [4]  Rogue
    150.375940f, // [5]  Priest
    88.129021f, // [6]  DK
    145.560408f, // [7]  Shaman
    150.375940f, // [8]  Mage
    150.375940f, // [9]  Warlock
    0.000000f, // [10] <Unused>
...
float Unit::CalculateEffectiveDodgeChance(const Unit* attacker, WeaponAttackType attType, const SpellEntry* ability) const
{
    ...
    if (!isPlayerOrPet && difference > 0)
        factor = 0.1f;
    // Dynamic diminishing for player's tanking gear
    if (GetTypeId() == TYPEID_PLAYER)
    {
        const uint8 pclass = getClass();
        if (pclass < MAX_CLASSES)
        {
            const float cap = AVOIDANCE_DIMINISHING_DODGE_CAP[pclass];
            const float k = AVOIDANCE_DIMINISHING_COEFFICIENT[pclass];
            if (cap >= 1 && k >= 0.001f)
            {
                float diminishing = m_modDodgeChanceDiminishing;
                // We need to diminish extra defense from rating only, before that it is just compensating for lack of skill
                if (difference > 0)
                {
                    const int32 extra = int32(((const Player*)this)->GetRatingBonusValue(CR_DEFENSE_SKILL));
                    const int32 amount = std::min(difference, extra);
                    // Substract it from difference to use in diminished formula
                    diminishing += (amount * factor);
                    difference -= amount;
                }
                const float nondiminishing = (chance - diminishing);
                chance = nondiminishing + ((diminishing * cap) / (diminishing + (cap * k)));
            }
        }
    }
...
{
    0.000000f, // [0]  <Unused>
    47.003525f, // [1]  Warrior
    47.003525f, // [2]  Paladin
    145.560408f, // [3]  Hunter
    145.560408f, // [4]  Rogue
    0.000000f, // [5]  Priest
    47.003525f, // [6]  DK
...
float Unit::CalculateEffectiveParryChance(const Unit* attacker, WeaponAttackType attType, const SpellEntry* ability) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (pclass < MAX_CLASSES)
        {
            const float cap = AVOIDANCE_DIMINISHING_PARRY_CAP[pclass];
            const float k = AVOIDANCE_DIMINISHING_COEFFICIENT[pclass];
            if (cap >= 1 && k >= 0.001f)
            {
                float diminishing = m_modParryChanceDiminishing;
                // We need to diminish extra defense from rating only, before that it is just compensating for lack of skill
                if (difference > 0)
                {
                    const int32 extra = int32(((const Player*)this)->GetRatingBonusValue(CR_DEFENSE_SKILL));
...
float Unit::GetHitChance(SpellSchoolMask schoolMask) const
{
    ...
    chance += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_INCREASES_SPELL_PCT_TO_HIT, schoolMask);
    return chance;
}
...
float Unit::CalculateEffectiveMissChance(const Unit* victim, WeaponAttackType attType, const SpellEntry* ability) const
{
    ...
    {
        const uint8 pclass = victim->getClass();
        if (pclass < MAX_CLASSES)
        {
            const float cap = AVOIDANCE_DIMINISHING_MISS_CAP[pclass];
            const float k = AVOIDANCE_DIMINISHING_COEFFICIENT[pclass];
            if (cap >= 1 && k >= 0.001f && difference > 0)
            {
                // We need to diminish extra defense from rating only, before that it is just compensating for lack of skill
                const int32 extra = int32(((const Player*)victim)->GetRatingBonusValue(CR_DEFENSE_SKILL));
                const int32 amount = std::min(difference, extra);
                // Substract it from difference to use in diminished formula
                float diminishing = (amount * factor);
                difference -= amount;
                const float nondiminishing = (chance - diminishing);
                chance = nondiminishing + ((diminishing * cap) / (diminishing + (cap * k)));
            }
...
float Unit::CalculateSpellCritChance(const Unit* victim, SpellSchoolMask schoolMask, const SpellEntry* spell) const
{
    ...
    for (AuraList::const_iterator i = scripts.begin(); i != scripts.end(); ++i)
    {
        ...
        switch ((*i)->GetModifier()->m_miscvalue)
        {
            ...
            // Glyph of Shadowburn
            case 7917: if (victim->HasAuraState(AURA_STATE_HEALTHLESS_35_PERCENT)) chance += (*i)->GetModifier()->m_amount; break;
            // Renewed Hope
            case 7997:
            case 7998: if (victim->HasAura(6788)) chance += (*i)->GetModifier()->m_amount; break;
            default:
                break;
        }
    }
            ...
            if (spell->IsFitToFamilyMask(uint64(0x0000000000000004)))
            {
                ...
                if (victim->GetAura(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, uint64(0x000000000000002), 0, GetObjectGuid()))
                {
                    ...
                    for (AuraList::const_iterator i = dummies.begin(); i != dummies.end(); ++i)
                    {
                        ...
                        if ((*i)->GetSpellProto()->SpellIconID == 1771)
                        {
                            chance += (*i)->GetModifier()->m_amount;
                            break;
                        }
...
float Unit::CalculateEffectiveMagicResistancePercent(const Unit* attacker, SpellSchoolMask schoolMask, bool binary) const
{
    ...
    float percent = float(float(resistance) / (float(skill) + resistance)) * 100;
...
uint32 Unit::MeleeDamageBonusTaken(Unit* pCaster, uint32 pdamage, WeaponAttackType attType, SpellEntry const* spellProto, DamageEffectType damagetype, uint32 stack)
{
    ...
    // differentiate for weapon damage based spells
    bool isWeaponDamageBasedSpell = !(spellProto && (damagetype == DOT || IsSpellHaveEffect(spellProto, SPELL_EFFECT_SCHOOL_DAMAGE)));
    uint32 schoolMask       = spellProto ? spellProto->SchoolMask : uint32(pCaster->GetMeleeDamageSchoolMask());
...
void CharmInfo::SetCharmState(std::string const& ainame /*= "PetAI"*/, bool withNewThreatList /*= true*/)
{
    ...
    const uint32 flags = (UNIT_FLAG_PLAYER_CONTROLLED | UNIT_FLAG_IMMUNE_TO_PLAYER | UNIT_FLAG_IMMUNE_TO_NPC);
...
void Unit::InterruptSpellsCastedOnMe(bool killDelayed)
{
    ...
    for (std::list<Unit*>::iterator iter = targets.begin(); iter != targets.end(); ++iter)
    {
        ...
        for (auto i_Events = (*iter)->m_Events.GetEvents().begin(); i_Events != (*iter)->m_Events.GetEvents().end(); ++i_Events)
